module ReleaseHelper
  def assert_tags( track_path, track, publisher, url )
    case File.extname( track_path )
      when '.ogg'
        assert_ogg_tags track_path, track, publisher, url
      when '.mp3'
        assert_mp3_tags track_path, track, publisher, url
      when '.flac'
        assert_flac_tags track_path, track, publisher, url
      else
        fail
    end
  end

  def assert_flac_tags( track_path, track, publisher, url )
    TagLib::FLAC::File.open( track_path ) do |file|
      ...
      assert_equal [url], tag.field_list_map['CONTACT']
      assert_equal [track.album.license.name], tag.field_list_map['LICENSE']
      assert_equal [publisher.name], tag.field_list_map['ORGANIZATION']
      assert_equal ["#{track.album.year} #{track.artist.name}"], tag.field_list_map['COPYRIGHT']
    end
  end

  def assert_ogg_tags( track_path, track, publisher, url )
  ...
  def assert_mp3_tags( track_path, track, publisher, url )
